...
class SynchedActorData {
   ...
   public:
    ...
    SynchedActorData(class SynchedActorData &&);
    SynchedActorData();
    bool _assignValues(class SynchedActorData const &, class Actor *);
    class SynchedActorData _clone() const;
    bool assignValues(std::vector<std::unique_ptr<class DataItem>> const &, class Actor *);
    // void forEachDataItem(class std::function<void(std::unique_ptr<class DataItem> const &)> const &) const;
    class CompoundTag const &getCompoundTag(unsigned short) const;
    float getFloat(unsigned short) const;
    int getInt(unsigned short) const;
    int64_t getInt64(unsigned short) const;
    signed char getInt8(unsigned short) const;
    class BlockPos getPosition(unsigned short) const;
    ...
    uint64_t getValidDataItemCount() const;
    class Vec3 getVec3(unsigned short) const;
    bool hasData(unsigned short) const;
    void markDirty(unsigned short);
    class SynchedActorData &operator=(class SynchedActorData &&);
    std::vector<std::unique_ptr<class DataItem>> packAll() const;
    std::vector<std::unique_ptr<class DataItem>> packDirty();
